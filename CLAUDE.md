...
- `src/itineraries.js` / `src/ItineraryBar.jsx` — curated journeys (5 routes, schema test-enforced: every stop a canon id + narration line). Bar under the globe: picker chip → active bar (name, stop k/n, ‹ › pacing, narration line); stepping selects the stop (existing fly-to). Globe draws the route as a dotted great-circle line through the stops (visited = filled dots, upcoming = rings) via the `itinerary` prop ({ids, upTo}) — drawn between land and influence arcs. Wandering to an on-route thinker syncs the bar index. No auto-advance by design.
...
- `src/agent.js` / `src/AgentPanel.jsx` — "Ask Philosophia" conversational agent (tested). **BYOK by owner decision**: user's Anthropic key in localStorage, browser→api.anthropic.com direct (`@anthropic-ai/sdk`, `dangerouslyAllowBrowser`) — the site is static; the ONLY backend is the optional X-publishing Worker (`worker/`, owner-requested 2026-07-21, see below), and the app stays fully keyless/functional without it. Don't add any other backend unasked. Three modes: resident guide (grounded via per-turn retrieval over search.js — persona/selection/last-cited/query-matched records ride in `<records>` on the user turn, system prompt holds rules + full roster), persona chat (entered via "Converse with X" button in DetailPanel), and the SYMPOSIUM ("symposium" link in panel tools): two personas debate, owner as questioner/third chair — each turn is one persona call built by `symposiumMessages` (speaker's own turns replay as assistant blocks, everyone else's words arrive as name-attributed user turns, records-bearing cue turn appended); user paces the debate (empty submit = "{next} responds", text = interjection); both chairs render living/static portraits with the speaker's loop pulsing; voices alternate per speaker; NO auto-run, NO auto-relisten in symposium. PUBLISH ("publish" link once any reply lands, in EVERY mode — guide, persona, and symposium): `src/broadsheet.js` renders the exchange as an engraved PNG broadsheet client-side (canvas, house palette, attributed turns — pure layout helpers incl. `fitTitleSpacing` tested). Cast decides layout: symposium = two medallions + "A SYMPOSIUM" title; solo persona/guide = one centered medallion + "A CONVERSATION" title (guide → `GUIDE`='philosophia' sentinel, Lady Philosophia's portrait/name; `sessionFromMessages` normalizes null→GUIDE, so renderBroadsheet never sees null). Title letter-spacing auto-fits so the longer conversation title can't overflow the frame; the question quote opens on the first wrapped line and closes on the last. Any voiced conversation (not just symposiums) stitches its ElevenLabs mp3 clips (ElevenSpeaker onClip tape, play order) into one audio file. Default ship path: navigator.share (share sheet → Messages/X) with a fallback menu (download image/audio, copy transcript, tweet-composer intent). OPTIONAL one-tap direct post to X: when the owner configures a publish endpoint + token in the keys form (`src/publish-api.js`), publish() POSTs the PNG + `captionFor` caption to the **Cloudflare Worker** in `worker/` (owner-requested 2026-07-21; `worker/SETUP.md` has the deploy steps). The Worker holds the four X OAuth 1.0a secrets + a shared PUBLISH_TOKEN gate (never in the bundle) and signs via `worker/x-oauth.js` (HMAC-SHA1, tested against the OAuth Core 1.0a A.5.2 vector); media upload = X v1.1 multipart, tweet = v2 JSON. On ANY failure it falls back to the share sheet (which keeps the image). **Known open question: image/media upload may require X's paid Basic tier (~$100/mo); text is free-tier — the app surfaces X's real error and falls back.** Everything unconfigured → share sheet only, exactly as before. Replies use the `[[id]]` marker protocol → clickable chips that select + fly the globe; unknown ids render literally; half-streamed markers hidden. Default model `claude-opus-4-8` (picker: sonnet-5, haiku-4-5; haiku gets NO thinking param — pre-adaptive). max_tokens 8000 = deliberate cost guard. The agent must never quiz — enforced in the system prompt; keep it that way. Panel medallion: guide mode wears `public/portraits/philosophia.jpg` (Lady Philosophia, generated 2026-07-20 with the standard Higgsfield template; full-res in ~/Desktop/Philosophy-portrait-originals), persona mode wears the thinker's existing thumb.
...
- `src/codex.js` / `src/CommonplaceBook.jsx` — the commonplace book: ❧ clip buttons on entry ideas/lines and agent replies file passages into localStorage; the book section (below the agent panel) appears once non-empty, entries carry a fly-to chip + remove. The guide searches it: `clippingsBlock(question)` appends word-overlap-matched clippings to guide/persona user turns (NOT symposium). Store dedupes by text; `philosophia:codex` window event syncs the UI. Tested.
- `src/DailyLine.jsx` — the day's line under the header: deterministic date-seeded pick (`dailyPick`, tested), signature line + name chip, dismissible per-day (localStorage).
